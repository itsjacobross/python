...
def midpoint_displacement(pt1, pt2, pt3, level=0, roughness=1.8, vertical_displacement=1):
    points = [pt1, pt2, pt3]
    ...
    if level == 0:
        if pt1 != start_1:
            pt1[2] += random.choice([-vertical_displacement, vertical_displacement])/3
        if pt2 != start_2:
            pt2[2] += random.choice([-vertical_displacement, vertical_displacement])/3
        if pt3 != start_3:
            pt3[2] += random.choice([-vertical_displacement, vertical_displacement])/3
        VERTICES.append(pt1)
        VERTICES.append(pt2)
        VERTICES.append(pt3)
...
midpoint_displacement(start_1, start_2, start_3, level=4)
print(VERTICES)
...
def main():
    ...
    while True:
        ...
        for vert3D in VERTICES:
            x, y, z = vert3D
            sx, sy = transform(x, y, z, angle, .707) #x,y,z,angle,0.25 #1.57 for topdown
            VERT2D.append((sx, sy))
